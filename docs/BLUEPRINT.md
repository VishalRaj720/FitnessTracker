...
## Appendix C — Decision reversals (23 Sep 2026)

Three decisions in the body of this document have been deliberately overturned. They are
recorded here rather than edited in place, so the original reasoning stays legible.

### C.1 The LLM ban is lifted — Part 7 (line ~225) and Part 25 (item 6)

**Was:** "LLM | Anything | **No** | Adds latency, cost, hallucination risk, and no core
value." and "LLM chatbot / AI trainer chat — undifferentiated, flaky in demos."

**Now:** a Gemini-backed coach, in three places: per-rep form cues during a workout, a Coach
tab for conversation about goals/streaks/plans, and a post-session debrief.

**Why the original objection no longer holds.** It was an objection to an LLM *chatbot* —
a bolt-on with no access to anything the app knows. What is built instead is fed the
**joint-angle time series** the analyzer already computes and throws away. That is not a
feature every team has, because it requires the CV engine to exist first. It can name
faults no `FormRule` encodes: tempo collapse, left/right asymmetry, a lean that creeps up
across a set, hips rising before the chest.

The three sub-objections are each answered structurally rather than argued away:

| Objection | Answer |
|---|---|
| Latency | The model is never in the frame loop. It is called once per rep, pipelined: fired when rep N completes, spoken during rep N+1. Deterministic cues stay instant. |
| Flaky in demos | With no API key the feature disables itself and the app behaves exactly as before. Offline, timed out, rate-limited or malformed: no cue is emitted and the built-in cues carry the set. There is no demo state in which a model failure is visible. |
| Hallucination | The model never sees a rule's verdict as fact, never receives client-claimed numbers as fact, and its output is validated against a fixed schema before rendering. Medical and nutrition questions never reach it at all (`companion/guard.py`). |

**Privacy is unchanged.** Part 0 and Part 19's "video never leaves the phone" still holds
literally: only derived joint angles and timings are transmitted. Sending camera frames
would have given better perception and was explicitly rejected for this reason.

### C.2 Demo clips are synthesised, not recorded — Part 8 tooling

The tutorial's 3D figure is generated by a forward-kinematics rig (`web/src/cv/demo/rig.ts`)
whose joint angles are derived from each exercise's own FSM thresholds, rather than recorded
from a person via `/dev/record`. This makes the demo *provably* show the depth the rules
demand — `demoClips.test.ts` asserts it by running the real `features()` over the generated
poses — which a recorded clip cannot guarantee. Clip recording was therefore not built; it
would be a second, unused, strictly worse data path.

### C.3 §8.3's decoupled overlay is now actually implemented

Part 8.3 promised "the overlay draws from the latest result independently, so UI stays at
60 fps even when inference is 15 fps". That was never true of the shipped code: inference
and drawing ran in the same tick, and inference ran synchronously on the main thread. It is
now true — inference is in a Web Worker, the overlay has its own rAF loop interpolating
between poses, and a `PerformanceGovernor` moves between quality tiers on measured cost.
§8.5's "Low fps devices → adaptive downscale" row is likewise now real rather than planned.
